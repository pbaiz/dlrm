...
def concatCriteoAdData(
    ...
    # Concatenates different n_data_split (days) and saves the result.
    #
    # Inputs:
    #   n_data_split (int): total number of n_data_split/days in the dataset (typically 7 or 24)
    #   d_path (str): path for {kaggle|terabyte}_split_i.npz files
    ...
    if memory_map:
        ...
        if data_split == "none":
            ...
            # randomize train data (per split)
            if randomize == "split":  # or randomize == "total":
                for i in range(len(indices) - 1):
                    indices[i] = np.random.permutation(indices[i])
                print("Randomized indices per split ...")

            train_indices = np.concatenate(indices[:-1])
            test_indices = indices[-1]

            # randomize train data (across n_data_split)
            if randomize == "total":
                train_indices = np.random.permutation(train_indices)
                print("Randomized indices across n_data_split ...")
        ...
        # on the split boundary del the file object and memmap again
        if recreate_flag:
            for i in range(n_data_split):
                filename_i = d_path + npzfile + "_{0}_reordered.npz".format(i)
            ...
            print("Reordered split files already exist, skipping ...")
        '''
        '''
        # Approach 2: group n_data_split
        # check if data already exists
        recreate_flag = False
        for j in range(tot_fea):
            filename_j = trafile + "_{0}_reordered.npy".format(j)
        ...
        if recreate_flag:
            ...
            group_split = 3  # e.g. 8, 4 or 3
            group_num = n_data_split // group_split
            file_group = [i*group_split for i in range(group_num)] + [n_data_split]
        ...
        if recreate_flag:
            ...
            for i in range(n_data_split):
                ...
                buckets = []
                for _j in range(n_data_split):
                    buckets.append([])
                counter = [0] * n_data_split
                n_data_split_to_sample = n_data_split if data_split == "none" else n_data_split - 1
                if randomize == "total":
                    rand_u = np.random.randint(low=0, high=n_data_split_to_sample, size=size)
                    for k in range(size):
                        # sample and make sure elements per buckets do not overflow
                        if data_split == "none" or i < n_data_split - 1:
                            # choose bucket
                            p = rand_u[k]
                            # retry of the bucket is full
                            while total_counter[p] + counter[p] >= total_per_file[p]:
                                p = np.random.randint(low=0, high=n_data_split_to_sample)
                        else:  # preserve the last split/bucket if needed
                            p = i
                        buckets[p].append(k)
                        counter[p] += 1
                else:  # randomize is split or none
                    for k in range(size):
        ...
        if recreate_flag:
            for j in range(n_data_split):
                ...
                if randomize == "split" or randomize == "total":
                    if data_split == "none" or j < n_data_split - 1:
        ...
        print("Concatenating multiple n_data_split into %s.npz file" % str(d_path + o_filename))
...
def transformCriteoAdData(X_cat, X_int, y, n_data_split, data_split, randomize, total_per_file):
    ...
    #     n_data_split     number of splits (e.g. days, typically between 7 - 24)
...
def getNormalData(
    ...
    npzfile = d_path + ((d_file + "_split") if criteo_kaggle else d_file)
    trafile = d_path + ((d_file + "_fea") if criteo_kaggle else "fea")

    # count number of datapoints in training set
    total_file = d_path + d_file + "_split_count.npz"
    if path.exists(total_file):
        ...
        if criteo_kaggle:
            ...
            if path.exists(datafile):
                ...
                num_data_per_split, extras = divmod(total_count, n_data_split)
                total_per_file = [num_data_per_split] * n_data_split
                for j in range(extras):
                    total_per_file[j] += 1
                # split into n_data_split (simplifies code later on)
                file_id = 0
            ...
            # WARNING: The raw data consist of split_0.gz,... ,split_23.gz text files
    ...
    # process all splits
    processCriteoAdData(d_path, d_file, npzfile, n_data_split, convertDicts, counts)
...
def loadDataset(
    ...
    if dataset == "normal":
        #n_data_split = 7
        o_filename = "normal_processed"
    elif dataset == "large":
        #n_data_split = 24
        o_filename = "large_processed"
    else:
    ...
    lstr = raw_path.split("/")
    d_path = "/".join(lstr[0:-1]) + "/"
    d_file = lstr[-1].split(".")[0] if dataset == "normal" else lstr[-1]
    npzfile = d_path + ((d_file + "_split") if dataset == "normal" else d_file)
    # trafile = d_path + ((d_file + "_fea") if dataset == "normal" else "fea")
