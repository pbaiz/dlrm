...
class DLRM_Model(object):
    ...
    def run(self):
        ...
        if self.args.mlperf_logging:
            print('command line args: ', json.dumps(vars(self.args)))

        ### some basic setup ###
        np.random.seed(self.args.numpy_rand_seed)
        np.set_printoptions(precision=self.args.print_precision)
        torch.set_printoptions(precision=self.args.print_precision)
        torch.manual_seed(self.args.numpy_rand_seed)

        if (self.args.test_mini_batch_size < 0):
            # if the parameter is not set, use the training batch size
            self.args.test_mini_batch_size = self.args.mini_batch_size
        if (self.args.test_num_workers < 0):
            # if the parameter is not set, use the same parameter for training
            self.args.test_num_workers = self.args.num_workers

        use_gpu = self.args.use_gpu and torch.cuda.is_available()
        if use_gpu:
            torch.cuda.manual_seed_all(self.args.numpy_rand_seed)
        ...
        if self.args.arch_interaction_op == "dot":
            # approach 1: all
            # num_int = num_fea * num_fea + m_den_out
            # approach 2: unique
            if self.args.arch_interaction_itself:
                num_int = (num_fea * (num_fea + 1)) // 2 + m_den_out
            else:
                num_int = (num_fea * (num_fea - 1)) // 2 + m_den_out
        elif self.args.arch_interaction_op == "cat":
            num_int = num_fea * m_den_out
        else:
            sys.exit(
                "ERROR: --arch-interaction-op="
                + self.args.arch_interaction_op
                + " is not supported"
            )
        ...
        if self.args.qr_flag:
            if self.args.qr_operation == "concat" and 2 * m_spa != m_den_out:
                sys.exit(
            ...
            if self.args.qr_operation != "concat" and m_spa != m_den_out:
                sys.exit(
        ...
        if self.args.md_flag:
            m_spa = md_solver(
                torch.tensor(ln_emb),
                self.args.md_temperature,  # alpha
                d0=m_spa,
                round_dim=self.args.md_round_dims
            ).tolist()
        ...
        ndevices = min(ngpus, self.args.mini_batch_size, num_fea - 1) if use_gpu else -1
        ...
        dlrm = DLRM_Net(
            ...
            ln_bot,
            ln_top,
            arch_interaction_op=self.args.arch_interaction_op,
            arch_interaction_itself=self.args.arch_interaction_itself,
            sigmoid_bot=-1,
            sigmoid_top=ln_top.size - 2,
            sync_dense_params=self.args.sync_dense_params,
            loss_threshold=self.args.loss_threshold,
            ndevices=ndevices,
            qr_flag=self.args.qr_flag,
            qr_operation=self.args.qr_operation,
            qr_collisions=self.args.qr_collisions,
            qr_threshold=self.args.qr_threshold,
            md_flag=self.args.md_flag,
            md_threshold=self.args.md_threshold,
        )
        # test prints
        if self.args.debug_mode:
        ...
        if self.args.loss_function == "mse":
            ...
            loss_ws = torch.tensor(np.fromstring(self.args.loss_weights, dtype=float, sep="-"))
            loss_fn = torch.nn.BCELoss(reduction="none")
        else:
            sys.exit("ERROR: --loss-function=" + self.args.loss_function + " is not supported")

        if not self.args.inference_only:
            # specify the optimizer algorithm
            optimizer = torch.optim.SGD(dlrm.parameters(), lr=self.args.learning_rate)
            lr_scheduler = LRPolicyScheduler(optimizer, self.args.lr_num_warmup_steps, self.args.lr_decay_start_step,
                                             self.args.lr_num_decay_steps)
        ...
        if not (self.args.load_model == ""):
            print("Loading saved model {}".format(self.args.load_model))
            if use_gpu:
                if dlrm.ndevices > 1:
                    # NOTE: when targeting inference on multiple GPUs,
                    # load the model as is on CPU or GPU, with the move
                    # to multiple GPUs to be done in parallel_forward
                    ld_model = torch.load(self.args.load_model)
                else:
                ...
                ld_model = torch.load(self.args.load_model, map_location=torch.device('cpu'))
            ...
            if not self.args.inference_only:
                ...
                self.args.print_freq = ld_nbatches
        ...
        with torch.autograd.profiler.profile(self.args.enable_profiling, use_gpu) as prof:
            while k < self.args.nepochs:
                ...
                for j, (X, lS_o, lS_i, T) in enumerate(self.train_ld):
                    ...
                    mbs = T.shape[0]  # = self.args.mini_batch_size except maybe for last
                    ...
                    should_print = ((j + 1) % self.args.print_freq == 0) or (j + 1 == nbatches)
                    should_test = (
                        (self.args.test_freq > 0)
                        and (self.args.data_generation == "dataset")
                        and (((j + 1) % self.args.test_freq == 0) or (j + 1 == nbatches))
                    )
                    ...
                    if should_test and not self.args.inference_only:
                        ...
                        if self.args.mlperf_logging:
                            ...
                            metrics = {
                                ...
                                'confusion_matrix': lambda y_true, y_score:
                            ...
                            for metric_name, metric_function in metrics.items():
                                ...
                                if metric_name not in ['classification_report', 'confusion_matrix']:
                                    print("{} {:.4f}".format(metric_name, 1000 * (met_time)), end="")
                            print(" ms")
                        ...
                        if self.args.mlperf_logging:
                            ...
                            is_best2 = validation_results['pre_auc'] > best_pre_auc_test
                            if is_best2:
                                best_pre_auc_test = validation_results['pre_auc']
                            print(
                        ...
                        if (self.args.mlperf_logging
                            and (self.args.mlperf_acc_threshold > 0)
                            and (best_gA_test > self.args.mlperf_acc_threshold)):
                            print("MLPerf testing accuracy threshold "
                                  + str(self.args.mlperf_acc_threshold)
                                  + " reached, stop training")
                            break

                        if (self.args.mlperf_logging
                            and (self.args.mlperf_auc_threshold > 0)
                            and (best_auc_test > self.args.mlperf_auc_threshold)):
                            print("MLPerf testing auc threshold "
                                  + str(self.args.mlperf_auc_threshold)
                                  + " reached, stop training")
                            break
        ...
        # plot compute graph
        if self.args.plot_compute_graph:
            #sys.exit(
            #    "ERROR: Please install pytorchviz package in order to use the"
            #    + " visualization. Then, uncomment its import above as well as"
            #    + " three lines below and run the code again."
            #)
            V = Z.mean() if self.args.inference_only else E
            dot = make_dot(V, params=dict(dlrm.named_parameters()))
            dot.render('dlrm_s_pytorch_graph') # write .pdf file

        # test prints
        if not self.args.inference_only and self.args.debug_mode:
        ...
        final_results = {
            ...
            'best_pre_auc_test': best_pre_auc_test
        }
        keys_keep = ['loss', 'recall', 'precision', 'f1', 'ap', 'roc_auc', 'pre_auc', 'accuracy', 'confusion_matrix',
