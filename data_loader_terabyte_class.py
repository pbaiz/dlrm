...
class DataLoader:
    ...
    def __init__(
        ...
        total_file = os.path.join(
            data_directory,
            data_filename + "_split_count.npz"
        )
        with np.load(total_file) as data:
            total_per_file = data["total_per_file"][np.array(n_data_split)]
    ...
    def __iter__(self):
        return iter(
            _batch_generator(
                self.data_filename, self.data_directory, self.n_data_split,
...
def _batch_generator(
        data_filename, data_directory, n_data_split, batch_size, split, drop_last, max_ind_range
):
    previous_file = None
    for split in n_data_split:
        filepath = os.path.join(
            data_directory,
            data_filename + "_{}_reordered.npz".format(split)
        )
...
def _test():
    generator = _batch_generator(
        ...
        n_data_split=range(n_data_split),
        split="train",
        batch_size=2048
    )
    t1 = time.time()
