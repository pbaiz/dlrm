...
import data_loader_terabyte_class
...
class NormalDataset(Dataset):
    ...
    def __init__(
        ...
        self.d_file = lstr[-1].split(".")[0] if dataset == "normal" else lstr[-1]
        self.npzfile = self.d_path + (
            (self.d_file + "_split") if dataset == "normal" else self.d_file
        )
        self.trafile = self.d_path + (
            (self.d_file + "_fea") if dataset == "normal" else "fea"
        )

        # check if pre-processed data is available
        data_ready = True
        if memory_map:
            for i in range(self.n_data_split):
        ...
        # get a number of samples per split
        total_file = self.d_path + self.d_file + "_split_count.npz"
        ...
        if memory_map:
            ...
            if split == 'none' or split == 'train':
                self.split = 0
                self.max_split_range = self.n_data_split if split == 'none' else self.v - 1
            elif split == 'test' or split == 'val':
                self.split = self.n_data_split - 1
                num_samples = self.offset_per_file[self.n_data_split] - \
                              self.offset_per_file[self.n_data_split - 1]
            ...
            # Load the test data
            # Only a single split is used for testing
    ...
    def __getitem__(self, index):
        ...
        if self.memory_map:
            if self.split == 'none' or self.split == 'train':
                # check if need to swicth to next split and load data
                if index == self.offset_per_file[self.split]:
                    # print("split_boundary switch", index)
                    self.split_boundary = self.offset_per_file[self.split]
                    fi = self.npzfile + "_{0}_reordered.npz".format(
                        self.split
                    )
                    ...
                    with np.load(fi) as data:
                        ...
                        self.X_cat = data["X_cat"]  # categorical feature
                        self.y = data["y"]          # target
                    self.split = (self.split + 1) % self.max_split_range

                i = index - self.split_boundary
...
def ensure_dataset_preprocessed(args, d_path):
    ...
    for split in ['train', 'val', 'test']:
        ...
        data_loader_terabyte_class.numpy_to_binary(input_files=input_files,
